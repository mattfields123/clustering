...
contains
    ...
    function velocity_pointML(x, y, t, phase1, phase2, time1, time2) result(velocity)
        ...
        velocity(1) = velocity(1) + (tausol*l - taupot*k*delta)*MaduLawrence(time1(counter_k,counter_l)-t,vu,mu)*psi1 
        velocity(1) = velocity(1) - taupot*(1-delta)*k*MaduLawrence(time2(counter_k,counter_l)-t,vu,mu)*psi2
        velocity(2) = velocity(2) - (tausol*k + taupot*l*delta)*MaduLawrence(time1(counter_k,counter_l)-t,vu,mu)*psi1 
        velocity(2) = velocity(2) - taupot*l*(1-delta)*MaduLawrence(time2(counter_k,counter_l)-t,vu,mu)*psi2
        end do
        end do
        !$OMP END PARALLEL DO
