...
subroutine vel_passive(timesteps, phase1, phase2, time1, time2, N_part)
    ...
    real :: rate
    integer :: end1, beg1, end2, beg3, end3
    ...
    open(1, file = 'velocity.dat', form='unformatted')
    open(2, file = 'particles.dat', form='unformatted')
    open(3, file = 'partavg.dat', form='unformatted')
    ...
    !print*, 'time to compute averages : ', real(end2-beginning) / real(rate)
    ...
    call system_clock(end1)
     ...
     print*, 'time to velocity computation : ', real(end1-beg1) / real(rate)



    call system_clock(beg3,rate)
    ...
    !$OMP END PARALLEL DO
    call system_clock(end3)

    print*,'time to particle : ', real(end3-beg3) / real(rate)

    ! writes the particle... What does the modulo do?
    ...
    ! Writes to the disk the Averages
...
subroutine super_passive(timesteps, phase1, phase2, time1, time2, N_part)
    ...
    open(1, file = 'bgvelocity.dat')
    open(2, file = 'bgparticles.dat')
    open(3, file = 'bgpartavg.dat')
    !open(4, file = 'betastreamfunction.dat')
    !open(7, file = 'betapotential.dat')
    !open(8, file = 'betadivergence.dat')
    ...
    close(2)
    close(1) 
    print*, 'Background flow experiment with ', N_part**2, ' particles'
